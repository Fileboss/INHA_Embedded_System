	...
	int main(void)
	{
		...
		while (1)
		{
            // Here we check at every clock cycle if one of the buttons is pressed
            // I think it's not a good idea, we's better use handlers
			if (GPIOA->IDR & GPIO_IDR_ID1) GPIOB->BSRR = GPIO_BSRR_BS2; // if button with id 1 is pressed, turn on the led with id 2
			else if (GPIOA->IDR & GPIO_IDR_ID2) GPIOE->BSRR = GPIO_BSRR_BS8; // if button with id 2 is pressed, turn on the led with id 8
			else if (GPIOA->IDR & GPIO_IDR_ID3) GPIOB->BSRR = GPIO_BSRR_BR2; // if button with id 3 is pressed, turn off the led with id 2
			else if (GPIOA->IDR & GPIO_IDR_ID5) GPIOE->BSRR = GPIO_BSRR_BR8; // if button with id 5 is pressed, turn off the led with id 8
			else if (GPIOA->IDR & GPIO_IDR_ID0) // when middle button is pressed, turn of both leds
			{
	...
	void ClockInit(void) // This method initialise the clock speed at its max value
	{
	...
	void GPIO_Init(GPIO_TypeDef *port, unsigned int pin, GPIO_Mode mode) // this method initialize GPIO pins
	{
